...
public class RepositoryConnectController implements IConnectedRepositoryInstance {
  ...
  public String createConnection() { //创建数据库链接
  ...
  public String editDatabaseConnection( String database ) { //编辑数据库链接
    CompletableFuture<String> future = new CompletableFuture<>();
    spoonSupplier.get().getShell().getDisplay().asyncExec( () -> {
  ...
  public boolean deleteDatabaseConnection( String database ) { //删除数据库链接
  ...
  public void connectToRepository( String repositoryName, String username, String password ) throws KettleException { //登录资源库
    RepositoriesMeta repositoriesMeta = getRepositoriesMeta();
    //查找资源库、获取资源bean
  ...
  public void test_connectToRepository(RepositoryMeta repositoryMeta, String username, String password, String token) throws KettleException { //测试后台连接资源库
	  final Repository repository = loadRepositoryObject( repositoryMeta.getId() );
  ...
  private void repositoryConnect( Repository repository, String username, String password ) throws KettleException { //资源库连接

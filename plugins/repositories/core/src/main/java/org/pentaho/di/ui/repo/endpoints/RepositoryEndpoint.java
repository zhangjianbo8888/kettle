...
import org.pentaho.di.ui.repo.utils.JwtPassport;
import org.pentaho.di.ui.repo.utils.JwtPayload;
...
public class RepositoryEndpoint {
   ...
   * 
   * @param loginModel
   * @param token 请求头中获取token
   * @return
   */
  @POST
  @Path( "/login_test" )
  @Consumes( { APPLICATION_JSON } )
  public Response login_aiyinsitan(@HeaderParam("token") String token) {
	LoginModel loginModel = new LoginModel();
	JwtPassport jwtPassport = new JwtPassport();
	JwtPayload jwtPayload = jwtPassport.getData(token);
	String userId = jwtPayload.getUserId();
	java.util.List<Map<String,String>> dataList = new ArrayList();
	int k = 0;
	for(Map<String, String> dataMap : dataList) {
		if(userId.equals(userId)){
			loginModel.setUsername(dataMap.get("userName"));
			loginModel.setPassword(dataMap.get("passWord"));
			k = 1;
			break;
		}
	}
	if (k == 0) {
		return Response.serverError().
				entity(new ErrorModel(BaseMessages.getString( PKG ,"无权访问"))).build();
	}
    try {
      if ( controller.isRelogin() ) {
        controller
